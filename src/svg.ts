import type { BiliBiliStats } from "./bilibili";
...
export function bilibiliCard(
  ...
  { colorPrimary = "#fb7299", colorText = "#333" }: RenderOptions = {},
): string {
  const normalizedLevel = Math.min(6, Math.max(0, Math.floor(level)));
  const levelColor = levelColors[normalizedLevel] ?? levelColors[0];
  const ringLength = 40 * 2 * Math.PI;
  const emptyRingPercent = 1 - normalizedLevel / 6;
    ...
    >
      <style>
        ...
        @keyframes rankAnimation {
          from { stroke-dashoffset: ${ringLength}; }
          to { stroke-dashoffset: ${ringLength * emptyRingPercent}; }
        }
      ...
      <g data-testid="card-title" transform="translate(22, 35)">
        <g transform="translate(0, 0)">
          ...
          <text x="0" y="0" class="header" data-testid="header" transform="translate(25, 0)">${escapeXml(username)}</text>
        </g>
      </g>

      <g transform="translate(0, 45)">
        <g class="stagger" style="animation-delay: 250ms" transform="translate(25, 0)">
          <text class="description" y="12.5">${escapeXml(description)}</text>
        </g>
      </g>

      <g data-testid="main-card-body" transform="translate(0, 55)">
        <g data-testid="rank-circle" transform="translate(400, 60)">
          <circle class="rank-circle-rim" cx="-10" cy="8" r="40" />
          <circle class="rank-circle" cx="-10" cy="8" r="40" />
          <g class="rank-text">
            <text x="-3" y="0" alignment-baseline="central" dominant-baseline="central" text-anchor="middle">
              Lv.${normalizedLevel}
            </text>
          </g>
        </g>
        ...
        <svg x="0" y="0">
          <g transform="translate(0, 25)">
            <g class="stagger" style="animation-delay: 450ms" transform="translate(25, 0)">
              ...
              <text class="stat" x="170" y="12.5" data-testid="stars">${followers}</text>
            </g>
          </g>
          <g transform="translate(0, 50)">
            <g class="stagger" style="animation-delay: 600ms" transform="translate(25, 0)">
              <text class="stat bold" y="12.5">Followings:</text>
              <text class="stat" x="170" y="12.5" data-testid="commits">${followings}</text>
            </g>
          </g>
          <g transform="translate(0, 75)">
            <g class="stagger" style="animation-delay: 750ms" transform="translate(25, 0)">
              <text class="stat bold" y="12.5">Total Videos:</text>
              <text class="stat" x="170" y="12.5" data-testid="issues">${videos}</text>
            </g>
          </g>
          <g transform="translate(0, 100)">
            <g class="stagger" style="animation-delay: 900ms" transform="translate(25, 0)">
              <text class="stat bold" y="12.5">Recent Views:</text>
              <text class="stat" x="170" y="12.5" data-testid="prs">${recentViews ?? "N/A"}</text>
            </g>
          </g>
        </svg>
      </g>
